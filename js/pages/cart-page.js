...
const CartPage = (() => {
  ...
  function renderSummary(cartItems) {
    ...
    if (discountEl) discountEl.innerText = `-${Products.formatPrice(0)}`;
    if (deliveryEl) deliveryEl.innerText = delivery === 0 ? "Free" : Products.formatPrice(delivery);
  ...
  function placeOrder() {
    ...
    setTimeout(() => {
      ...
      if (result.ok) {
        ...
        if (confirmEl) confirmEl.innerText = result.order.id.slice(-8).toUpperCase();
