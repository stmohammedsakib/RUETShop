...
const ProductsPage = (() => {
  ...
  function applyFilters() {
    ...
    switch (state.sort) {
      ...
      case "newest":
        ...
        break;
      default:
        // Default sort: by sold count (popular)
    ...
    return `
      <div class="product-card" data-id="${p.id}">
        ...
        <div class="product-card-body">
          ...
          <a href="product-detail.html?id=${p.id}" class="product-card-name">${escapeHtml(p.name)}</a>
  ...
  function renderActiveFilters() {
    ...
    if (state.search) {
      tags.push(`<span class="active-filter-tag">Search: "${state.search}" <button onclick="ProductsPage.clearSearch()">✕</button></span>`);
    }
    if (state.category) {
      tags.push(`<span class="active-filter-tag">${state.category} <button onclick="ProductsPage.clearCategory()">✕</button></span>`);
    }
    if (state.inStock) {
  ...
  function onPriceChange(val) {
    state.maxPrice = parseFloat(val) || 5000;
    const display = document.getElementById("priceVal");
    if (display) display.textContent = `৳${state.maxPrice}`;
  }
  ...
  function clearFilters() {
    ...
    if (priceVal) priceVal.textContent = "৳5000";
