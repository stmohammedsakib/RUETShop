...
// Fixed: added backward-compat aliases (add, getByProduct, remove)
// ============================================================
...
const Ratings = (() => {
  ...
  function addReview({ productId, rating, comment }) {
    ...
    const existing = DB.findOneWhere(COL, r =>
      r.productId === productId && r.userId === session.id
    );
  ...
  function getUserReviews(userId) {
    return DB.findWhere(COL, r => r.userId === userId)
             .sort((a, b) => b.createdAt - a.createdAt);
  }
  ...
  return {
    ...
    // product-detail.js (old version) calls these names:
    add:          (...args) => addReview(...args),
    getByProduct: (productId) => getProductReviews(productId),
    remove:       (reviewId)  => deleteReview(reviewId),
  };
})();
