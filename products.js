...
const Products = (() => {
  ...
  function create({ name, description, price, originalPrice, category, stock, images = [], tags = [] }) {
    ...
    if (!session || session.role !== "vendor") return { ok: false, error: "Only vendors can add products." };
  ...
  function update(id, updates) {
    ...
    if (product.vendorId !== session.id) return { ok: false, error: "You can only edit your own products." };
    ...
    if (clean.price) clean.price = parseFloat(clean.price);
    if (clean.originalPrice) clean.originalPrice = parseFloat(clean.originalPrice);
  ...
  function remove(id) {
    ...
    const product = getById(id);
    if (!product) return { ok: false, error: "Product not found." };
    if (product.vendorId !== session.id) return { ok: false, error: "You can only delete your own products." };
